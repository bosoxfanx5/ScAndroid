...
class ResponseParser {
	 ...
	 */
	public Object parse(InputStream response, boolean debugMode) throws de.timroes.axmlrpc.XMLRPCException {
		...
		try {
			...
			// Check for root tag
			if(!e.getNodeName().equals(de.timroes.axmlrpc.XMLRPCClient.METHOD_RESPONSE)) {
				throw new de.timroes.axmlrpc.XMLRPCException("MethodResponse root tag is missing.");
			}

			e = de.timroes.axmlrpc.XMLUtil.getOnlyChildElement(e.getChildNodes());

			if(e.getNodeName().equals(de.timroes.axmlrpc.XMLRPCClient.PARAMS)) {

				e = de.timroes.axmlrpc.XMLUtil.getOnlyChildElement(e.getChildNodes());

				if(!e.getNodeName().equals(de.timroes.axmlrpc.XMLRPCClient.PARAM)) {
					throw new de.timroes.axmlrpc.XMLRPCException("The params tag must contain a param tag.");
				}

				return getReturnValueFromElement(e);

			} else if(e.getNodeName().equals(de.timroes.axmlrpc.XMLRPCClient.FAULT)) {

				@SuppressWarnings("unchecked")
				Map<String,Object> o = (Map<String,Object>)getReturnValueFromElement(e);

				throw new de.timroes.axmlrpc.XMLRPCServerException((String)o.get(FAULT_STRING), (Integer)o.get(FAULT_CODE));

			}

			throw new de.timroes.axmlrpc.XMLRPCException("The methodResponse tag must contain a fault or params tag.");

		} catch(de.timroes.axmlrpc.XMLRPCServerException e) {
			throw e;
		} catch (Exception ex) {
			throw new de.timroes.axmlrpc.XMLRPCException("Error getting result from server.", ex);
		}
	 ...
	 */
	private Object getReturnValueFromElement(Element element) throws de.timroes.axmlrpc.XMLRPCException {
